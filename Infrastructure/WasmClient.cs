...
namespace EMMA.PluginTemplate.Infrastructure
{
    internal sealed class WasmClient
    {
        ...
        public VideoStreamOperationItem[] VideoStreams(string mediaId, string payloadJson)
        {
            ...
            var result = new List<VideoStreamOperationItem>(streams.Count);
            foreach (var stream in streams)
            {
                result.Add(new VideoStreamOperationItem(stream.Id, stream.Label, stream.PlaylistUri));
            }

            return [.. result];
        }

        public VideoSegmentOperationItem? VideoSegment(string mediaId, string streamId, uint sequence, string payloadJson)
        {
            ...
            return new VideoSegmentOperationItem(
                fixture.Value.ContentType,
                Convert.ToBase64String(fixture.Value.Payload));
        }
...
namespace LibraryWorld.wit.exports.emma.plugin
{
    public static class PluginImpl
    {
        ...
        public static IPlugin.VideoSegmentItem? VideoSegment(string mediaId, string streamId, uint sequence, string payloadJson)
        {
            ...
            return new IPlugin.VideoSegmentItem(
                segment.contentType,
                Convert.FromBase64String(segment.payload));
        }
